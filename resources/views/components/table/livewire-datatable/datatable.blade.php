                ...
                @if($this->searchableColumns()->count())
                    <div class="w-96 flex rounded">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <x-input-base wire:model.debounce.500ms="search"  placeholder="Cerca in {{ $this->searchableColumns()->map->label->join(', ') }}" />
                ...
                <div class="table align-middle min-w-full">
                    @unless($this->hideHeader)
                        <div class="table-row divide-x divide-neutral-200">
                            @foreach($this->columns as $index => $column)
                                @if($hideable === 'inline')
                                    @include('drystack::components.table.livewire-datatable.header-inline-hide', ['column' => $column, 'sort' => $sort])
                                ...
                                @else
                                    @include('drystack::components.table.livewire-datatable.header-no-hide', ['column' => $column, 'sort' => $sort])
                                @endif
                            @endforeach
                        </div>
